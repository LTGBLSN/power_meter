...
{      			    
  ...
  for(i=0;i<4;i++) //循环16次，画第一页的16列
	{
      OLED_Set_Pos(x,y+i);
			for(t=0;t<48;t++)	//循环16次，画第二页的16列
			{	
				OLED_WR_Byte(hanzi48[no][(48*i)+t],OLED_DATA);
			}	
	}	
}
  ...
  for(i=0;i<4;i++) //循环16次，画第一页的16列
	{
      OLED_Set_Pos(x,y+i);
			...
			{	
				OLED_WR_Byte(zifu48[no][(16*i)+t],OLED_DATA);
			}	
	}	
}

//硬件SPI初始化(speed值取:0/1/2/3,分别对应晶振的:4/16/64/128分频)
